"""Generacion de respuestas corporativas."""
...
class EmailResponder:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self.email_template = Template((settings.templates_dir / "email_respuesta.html").read_text(encoding="utf-8"))
    ...
    def generate(
        ...
        if self.settings.anthropic_api_key:
            try:
                return self._generate_with_claude(email, classification, product_context)
            except Exception:
                logger.exception("Claude fallo generando respuesta; usando fallback")

        body = self.email_template.render(cuerpo=self._build_mock_body(classification, product_context))
        return EmailResponse(
            email_id=email.id,
            to_email=email.from_email,
            subject=f"Re: {email.subject}",
            body=body,
            escalated=False,
            metadata={"classification": classification.categoria, "matches": len(product_context.matches)},
        )
    ...
    def _generate_with_claude(
        ...
        """Usa Claude para redactar una respuesta."""
        ...
        client = Client(api_key=self.settings.anthropic_api_key)
