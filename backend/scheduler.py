...
def _should_run_stock(settings) -> bool:
    """Solo ejecuta stock si hay CSV de ventas real o si se fuerza."""
    return not settings.mock_mode or settings.csv_ventas.exists()
...
def start_scheduler() -> None:
    ...
    if not _should_run_stock(settings):
        logger.info("Modo mock — scheduler de stock desactivado (sin CSV de ventas)")
    else:
        scheduler.add_job(
    ...
    logger.info("Scheduler iniciado: emails cada 10 min" + (f", stock diario a las %02d:%02d" % (hour, minute) if _should_run_stock(settings) else ""))

    _safe_job("emails", run_emails)
    if _should_run_stock(settings):
