...
export function Badge({ children, variant = 'default' }: { children: ReactNode; variant?: BadgeVariant }) {
  return (
    <span className={clsx('inline-flex items-center px-2 py-0.5 rounded text-[11px] font-medium leading-tight whitespace-nowrap', badgeStyles[variant])}>
      {children}
    </span>
  )
}
...
export function DaysBar({ dias, leadTime }: { dias: number; leadTime: number }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-[90px]">
      ...
      <span className={clsx('text-[11px] tabular-nums w-6 text-right font-medium',
...
export function Th({ children, right }: { children?: ReactNode; right?: boolean }) {
  return (
    <th className={clsx('px-3 py-2 text-[11px] font-medium text-stone-400 uppercase tracking-wide border-b border-stone-100 bg-stone-50/60', right && 'text-right')}>
      {children}
    </th>
  )
}
...
export function Toast({ message, variant = 'info', onClose }: {
  ...
  return (
    <div className={clsx('toast-enter fixed bottom-5 right-5 z-[100] flex items-center gap-3 px-4 py-3 rounded-lg text-white text-[12px] font-medium shadow-lg min-w-[220px]', styles[variant])}>
      <span className="flex-1">{message}</span>
      <button onClick={onClose} className="opacity-70 hover:opacity-100 transition-opacity text-xs">✕</button>
    </div>
  )
}
...
export function SearchInput({ value, onChange, placeholder = 'Buscar...' }: {
  ...
  return (
    <div className="flex items-center gap-2 bg-stone-50 border border-stone-200 rounded-md px-3 py-1.5">
      <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="text-stone-400 flex-shrink-0">
      ...
      />
      {value && (
        <button onClick={() => onChange('')} className="text-stone-400 hover:text-stone-600 transition-colors">
...
export function FilterTabs<T extends string>({ options, value, onChange }: {
  ...
  return (
    <div className="flex items-center gap-px border border-stone-200 rounded-md overflow-hidden">
      {options.map(opt => (
        <button
          key={opt.value}
...
export function EmptyState({ message = 'No se encontraron resultados.' }: { message?: string }) {
  return (
    <div className="px-4 py-10 text-center">
      <p className="text-[12px] text-stone-400">{message}</p>
    </div>
  )
}
