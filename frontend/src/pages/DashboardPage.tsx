...
import { MetricCard, Card, Badge, SectionHeader, TableWrapper, Th, Td, Skeleton } from '../components/ui'
...
export default function DashboardPage() {
  ...
  const runEmails = async () => {
    ...
    if (res.ok) setLastRunEmail(new Date().toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' }))
    setRunning('idle')
    load()
  }
  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-base font-semibold text-stone-900">Panel de control</h1>
          <p className="text-[12px] text-stone-400 mt-0.5">Resumen operativo de Palmo IA</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-5 gap-3 mb-5">
          <MetricCard label="Email procesados" value={m?.emails_procesados_hoy ?? '-'} sub={lastRunEmail ? `último: ${lastRunEmail}` : 'hoy'} />
          <MetricCard label="Escalados" value={m?.emails_escalados_hoy ?? '-'} sub="requieren atención" accent={(m?.emails_escalados_hoy ?? 0) > 8 ? 'warning' : 'default'} />
          <MetricCard label="En riesgo" value={m?.productos_en_riesgo ?? '-'} sub="críticos + altos" accent={(m?.productos_en_riesgo ?? 0) > 10 ? 'danger' : 'warning'} />
          <MetricCard label="Precisión IA" value={m ? `${m.precision_ia}%` : '-'} sub="últimas 500" accent="success" />
          <MetricCard label="Alertas activas" value={m?.alertas_activas ?? '-'} sub="sin leer" accent={(m?.alertas_activas ?? 0) > 0 ? 'warning' : 'default'} />
        </div>
      )}
              ...
              {recentEmails.map(email => (
                <tr key={email.id} className="hover:bg-stone-50 transition-colors">
                  ...
                  <Td>
                    ...
                    <p className="text-[11px] text-stone-400">{email.empresa}</p>
                  </Td>
                  <Td><CategoriaBadge tipo={email.tipo} /></Td>
                  <Td>
                    <div className="flex items-center gap-2 min-w-[80px]">
                      <div className="flex-1 h-1 bg-stone-200 rounded-full overflow-hidden">
                        <div className={`h-full rounded-full ${email.confianza >= 0.9 ? 'bg-emerald-500' : email.confianza >= 0.8 ? 'bg-amber-400' : 'bg-red-400'}`}
                          style={{ width: `${Math.round(email.confianza * 100)}%` }} />
                      </div>
                      <span className="text-[11px] text-stone-500 tabular-nums w-7 text-right">{Math.round(email.confianza * 100)}%</span>
                    </div>
                  </Td>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { icon: Package, label: 'Stock', value: m?.total_productos ?? '-', sub: 'SKUs monitorizados', extra: `${stockData.filter(s => s.riesgo === 'critica').length} críticos · ${stockData.filter(s => s.riesgo === 'alta').length} altos` },
          { icon: Mail, label: 'Emails', value: m?.emails_procesados_hoy ?? '-', sub: 'procesados hoy', extra: m ? `${((m.emails_procesados_hoy - m.emails_escalados_hoy) / Math.max(m.emails_procesados_hoy, 1) * 100).toFixed(0)}% automáticos` : '' },
          { icon: Bell, label: 'Alertas', value: m?.alertas_activas ?? '-', sub: 'pendientes de revisión' },
          { icon: Activity, label: 'Sistema', value: 'Palmo IA', sub: `v1.0.0 · ${m?.mock_mode ? 'Modo demo' : 'Producción'}` },
        ].map(card => (
...
function CategoriaBadge({ tipo }: { tipo: string }) {
  ...
  const { label, variant } = map[tipo] ?? { label: tipo, variant: 'muted' }
...
function EstadoBadge({ estado }: { estado: string }) {
  ...
  const { label, variant } = map[estado] ?? { label: estado, variant: 'muted' }
  return <Badge variant={variant}>{label}</Badge>
}
...
function PrioridadBadge({ prioridad }: { prioridad: string }) {
  ...
  const { label, variant } = map[prioridad] ?? { label: prioridad, variant: 'muted' }
  return <Badge variant={variant}>{label}</Badge>
}
